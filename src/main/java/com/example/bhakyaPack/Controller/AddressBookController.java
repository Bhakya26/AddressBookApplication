...
public class AddressBookController {
    ...
    @PostMapping
    public ResponseEntity<String> create(@RequestBody AddressBookmodel entry) {
        return ResponseEntity.ok("POST: Creating new entry for " + entry.getName());
    }

    // PUT to update entry by ID
    @PutMapping("/{id}")
    public ResponseEntity<String> update(@PathVariable int id, @RequestBody AddressBookmodel updatedEntry) {
